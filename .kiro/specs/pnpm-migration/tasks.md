...
- [x] 1. Remove npm artifacts and set up pnpm configuration files
  - [x] 1.1 Delete `nodejs-api/package-lock.json` and `nodejs-api/node_modules/`
    - Remove the npm lockfile and existing node_modules directory
    - _Requirements: 1.1, 1.2, 2.1_

  - [x] 1.2 Create `nodejs-api/.npmrc` with pnpm settings
  ...
  - [x] 1.3 Create `nodejs-api/pnpm-workspace.yaml` with security settings
  ...
  - [x] 1.4 Update `nodejs-api/.gitignore` for pnpm
...
- [x] 2. Update `nodejs-api/package.json` for migration
  - [x] 2.1 Rename package and update namespace fields
  ...
  - [x] 2.2 Update engines, packageManager, and lifecycle scripts
  ...
  - [x] 2.3 Promote build dependencies and pin versions
  ...
  - [x] 2.4 Update `files` field for source distribution
...
- [x] 3. Checkpoint - Verify package.json and config files
  - Ensure all configuration files are correct, ask the user if questions arise.

- [x] 4. Harden DLL loading in `nodejs-api/addon.cc`
  - [x] 4.1 Replace bare `LoadLibraryA` call with absolute path resolution
...
- [x] 5. Create postinstall build script and update Go module
  - [x] 5.1 Create `nodejs-api/postinstall.mjs` — consumer-side build pipeline
  ...
  - [x] 5.4 Update `go.mod` module path to canonical GitHub organization path
    - Change `module pulsyflux` to `module github.com/pulsyflux/broker`
    - _Requirements: 19.1, 23.1, 23.4, 30.4, 30.7_

  - [x] 5.5 Update Go import paths in `broker_lib.go`
    - Change `import "pulsyflux/broker"` to `import "github.com/pulsyflux/broker/broker"`
    - _Requirements: 19.2_

  - [x] 5.6 Update Go import paths in `broker/server.go` and `broker/client.go`
...
- [x] 6. Checkpoint - Verify postinstall script and Go module changes
  - Ensure all tests pass, ask the user if questions arise.

- [x] 7. Run pnpm install and verify dependency resolution
  - [x] 7.1 Run `pnpm install` in `nodejs-api/` and verify lockfile generation
  ...
  - [x] 7.2 Verify build pipeline compatibility
    - Run `node build.mjs` to confirm C++ addon compiles
    - Run `pnpm exec jasmine` to confirm test suite passes
    - _Requirements: 7.1, 7.2_

- [x] 8. Create GitHub Actions CI/CD workflow
  - [x] 8.1 Create `.github/workflows/ci.yml` with Go stage
  ...
  - [x] 8.2 Add Node.js stage to CI workflow
    - Depends on Go stage
...
- [x] 9. Checkpoint - Verify CI workflow structure
  - Ensure all tests pass, ask the user if questions arise.

- [x] 10. Create integration test directory and tests
  - [x] 10.1 Create `test/go/` — Go consumer verification tests
  ...
  - [x] 10.2 Create `test/nodejs/` — Node.js consumer verification tests
  ...
  - [x] 10.3 Create integration test runner script
...
- [x] 11. Add Act support for local CI testing
  - [x] 11.1 Add Act configuration and documentation
...
- [x] 12. Final checkpoint - Verify complete migration
  - Ensure all tests pass, ask the user if questions arise.
