package socket

import (
	"bytes"
	"errors"
	"io"
	"net"
	"sync"
	"time"

	"github.com/google/uuid"
)

type Client struct {
	addr     string
	ctx      *connctx
	connMu   sync.Mutex
	clientID uuid.UUID
	peerID   uuid.UUID
	done     chan struct{}
	incoming chan *frame
	wg       sync.WaitGroup
}

func NewClient(addr string, peerID uuid.UUID) (*Client, error) {
	conn, err := net.Dial("tcp4", addr)
	if err != nil {
		return nil, err
	}
	if tcpConn, ok := conn.(*net.TCPConn); ok {
		tcpConn.SetNoDelay(true)
		tcpConn.SetReadBuffer(2 * 1024 * 1024)
		tcpConn.SetWriteBuffer(2 * 1024 * 1024)
	}

	c := &Client{
		addr:     addr,
		clientID: uuid.New(),
		peerID:   peerID,
		done:     make(chan struct{}),
		incoming: make(chan *frame, 100),
		ctx: &connctx{
			conn:   conn,
			writes: make(chan *frame, 1024),
			errors: make(chan *frame, 256),
			closed: make(chan struct{}),
			wg:     &sync.WaitGroup{},
		},
	}

	c.ctx.wg.Add(1)
	go c.ctx.startWriter()

	c.wg.Add(1)
	go c.readLoop()

	return c, nil
}

func (c *Client) readLoop() {
	defer c.wg.Done()
	for {
		select {
		case <-c.done:
			return
		default:
		}

		f, err := c.ctx.readFrame()
		if err != nil {
			return
		}

		select {
		case c.incoming <- f:
		case <-c.done:
			putFrame(f)
			return
		}
	}
}

func (c *Client) Send(r io.Reader, timeout time.Duration) (io.Reader, error) {
	reqID := uuid.New()
	timeoutMs := uint64(timeout.Milliseconds())
	if timeoutMs == 0 {
		timeoutMs = uint64(defaultTimeout.Milliseconds())
	}

	// Send startFrame and wait for peer's startFrame
	if err := c.sendStartFrame(reqID, timeoutMs); err != nil {
		return nil, err
	}
	if err := c.waitForStartFrame(reqID, timeout); err != nil {
		return nil, err
	}

	// Send chunks
	if err := c.sendChunks(reqID, r); err != nil {
		return nil, err
	}

	// Send endFrame
	if err := c.sendEndFrame(reqID, timeoutMs); err != nil {
		return nil, err
	}

	// Receive response
	return c.receiveResponse(reqID, timeout)
}

func (c *Client) Receive() (uuid.UUID, io.Reader, bool) {
	streamReqs := make(map[uuid.UUID]*bytes.Buffer)

	for {
		select {
		case f, ok := <-c.incoming:
			if !ok {
				return uuid.Nil, nil, false
			}

			switch f.Type {
			case startFrame:
				streamReqs[f.RequestID] = bytes.NewBuffer(make([]byte, 0, 1024))
				putFrame(f)

			case chunkFrame:
				buf := streamReqs[f.RequestID]
				if buf != nil {
					buf.Write(f.Payload)
				}
				putFrame(f)

			case endFrame:
				buf := streamReqs[f.RequestID]
				delete(streamReqs, f.RequestID)
				reqID := f.RequestID
				putFrame(f)
				if buf != nil {
					return reqID, bytes.NewReader(buf.Bytes()), true
				}

			case errorFrame:
				putFrame(f)
			}

		case <-c.done:
			return uuid.Nil, nil, false
		}
	}
}

func (c *Client) sendStartFrame(reqID uuid.UUID, timeoutMs uint64) error {
	startF := getFrame()
	startF.Version = version1
	startF.Type = startFrame
	startF.RequestID = reqID
	startF.ClientID = c.clientID
	startF.PeerClientID = c.peerID
	startF.ClientTimeoutMs = timeoutMs
	if !c.ctx.enqueue(startF) {
		putFrame(startF)
		return errors.New("failed to enqueue start frame")
	}
	return nil
}

func (c *Client) waitForStartFrame(reqID uuid.UUID, timeout time.Duration) error {
	deadline := time.After(timeout)
	for {
		select {
		case <-deadline:
			return errTimeout
		case f := <-c.incoming:
			if f.RequestID == reqID && f.Type == startFrame {
				putFrame(f)
				return nil
			}
			if f.RequestID == reqID && f.Type == errorFrame {
				putFrame(f)
				return errPeerError
			}
			// Put frame back for Receive
			select {
			case c.incoming <- f:
			default:
				putFrame(f)
			}
		case <-c.done:
			return errClosed
		}
	}
}

func (c *Client) sendChunks(reqID uuid.UUID, r io.Reader) error {
	buf := getBuffer()
	defer putBuffer(buf)

	chunk := getFrame()
	chunk.Version = version1
	chunk.Type = chunkFrame
	chunk.RequestID = reqID
	chunk.ClientID = c.clientID
	chunk.PeerClientID = c.peerID
	defer putFrame(chunk)

	for {
		n, err := r.Read(*buf)
		if n > 0 {
			chunk.Payload = (*buf)[:n]
			if !c.ctx.enqueue(chunk) {
				return errors.New("failed to enqueue chunk frame")
			}
		}
		if err == io.EOF {
			break
		}
		if err != nil {
			return err
		}
	}
	return nil
}

func (c *Client) sendEndFrame(reqID uuid.UUID, timeoutMs uint64) error {
	endF := getFrame()
	endF.Version = version1
	endF.Type = endFrame
	endF.RequestID = reqID
	endF.ClientID = c.clientID
	endF.PeerClientID = c.peerID
	endF.ClientTimeoutMs = timeoutMs
	if !c.ctx.enqueue(endF) {
		putFrame(endF)
		return errors.New("failed to enqueue end frame")
	}
	return nil
}

func (c *Client) receiveResponse(reqID uuid.UUID, timeout time.Duration) (io.Reader, error) {
	deadline := time.After(timeout)
	buf := getBuffer()
	payload := (*buf)[:0]

	for {
		select {
		case <-deadline:
			putBuffer(buf)
			return nil, errTimeout
		case f := <-c.incoming:
			if f.RequestID != reqID {
				// Put back for Receive
				select {
				case c.incoming <- f:
				default:
					putFrame(f)
				}
				continue
			}

			switch f.Type {
			case startFrame:
				putFrame(f)
			case chunkFrame:
				payload = append(payload, f.Payload...)
				putFrame(f)
			case endFrame:
				result := make([]byte, len(payload))
				copy(result, payload)
				putBuffer(buf)
				putFrame(f)
				return bytes.NewReader(result), nil
			case errorFrame:
				putBuffer(buf)
				putFrame(f)
				return nil, errPeerError
			}
		case <-c.done:
			putBuffer(buf)
			return nil, errClosed
		}
	}
}

func (c *Client) Close() error {
	close(c.done)
	c.wg.Wait()
	close(c.incoming)
	
	c.connMu.Lock()
	defer c.connMu.Unlock()
	
	if c.ctx != nil {
		close(c.ctx.writes)
		close(c.ctx.errors)
		c.ctx.wg.Wait()
		if c.ctx.conn != nil {
			err := c.ctx.conn.Close()
			c.ctx.conn = nil
			return err
		}
	}
	return nil
}
